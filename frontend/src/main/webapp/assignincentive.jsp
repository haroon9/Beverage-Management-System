...
<head>
    ...
    <title>Assign Incentive to the Beverage</title>
...
<div class="container shadow rounded mt-3 p-3">
    ...
    <h1 class="display-4">Assign Incentive to Beverage</h1>
    <span class="table-add float-right mb-3 mr-2"><a href="#!" class="text-success"><i
            class="fas fa-plus fa-2x" aria-hidden="true"></i></a></span>

    <form role="form" action="/frontend/beverages/assign" method="post">
        <div class="form-group">
            <label for="name">Beverage name</label>
            <input name="name" id="name" type="text" class="form-control" placeholder="<%=beverage.getName() %>"
                   disabled>
        </div>
        <div class="form-group">
            <label for="manufacturer">Manufacturer</label>
            <input name="manufacturer" id="manufacturer" type="text" class="form-control"
                   placeholder="<%= beverage.getManufacturer() %>" disabled>
        </div>

        <div class="form-group">
            <label for="quantity">Quantity</label>
            <input name="quantity" id="quantity" type="text" class="form-control"
                   placeholder="<%= beverage.getQuantity() %>"
                   disabled>
        </div>
        <div class="form-group">
            <label for="price">Price</label>
            <input name="price" id="price" type="text" class="form-control"
                   placeholder="<%= String.format("%.2f",beverage.getPrice()) %>"
                   disabled>
        </div>
